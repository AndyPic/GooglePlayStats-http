    ...
    <div class="container-fullscreen">
        ...
        <div class="pitch-container">
            <div class="pitch-title">
                <h2>What is your app idea?</h2>
            </div>

            <form method="POST"  id="pitch-form" action="create_pitch.php">
                <input id="pitch-title-input" type="text" name="pitch_title" title="Pitch title" placeholder="Enter a title for you pitch here" maxlength="64" required />
                <input id="app-name-input" type="text" name="name" title="App name" placeholder="App Name" maxlength="64" required />
                <div class="input-container posRel">
                    <input class="input-dropdown-input" id="app-genre-input" list="genre-dropdown" type="text" name="genre_id" title="App genre" placeholder="App Genre" maxlength="32" required />
                    <div class="popup">
                        <span class="popuptext">Invalid genre.</span>
                    </div>
                    <div class="search-container" id="genre-dropdown"></div>
                </div>

                <textarea name="description" id="app-description-input" rows="10" title="App description" placeholder="Describe how you imagine the app to be" maxlength="1024" required></textarea>
                <textarea name="motive" id="motivation-input" rows="10" title="Motive for the app" placeholder="Why do you think this would be a good app?" maxlength="1024" required></textarea>
                <textarea name="audience" id="target-audience-input" rows="5" title="Target audience" placeholder="Who do you think would use the app?" maxlength="512" required></textarea>

                <input type="hidden" name="user_id" value="<?= $_SESSION['id'] ?>">

                <button type="button" id="pitch-submit">SUBMIT</button>
            </form>
        </div>

        <script>
            var submitButton = document.getElementById("pitch-submit")
            var genreInput = document.getElementById("app-genre-input")

            submitButton.onclick = thisEvent => {
                var pitchForm = document.getElementById("pitch-form")
                // Check all required fields filled
                if (pitchForm.checkValidity()) {
                    // Check that genre correct
                    for (loop = 0; loop < genres['data'].length; loop++) {
                        if (genres['data'][loop]['genre'] == genreInput.value) {
                            genreInput.value = genres['data'][loop]['id']
                            pitchForm.submit()
                            break
                        }
                    }
                    runPopup(genreInput, 1)
                } else {
                    pitchForm.reportValidity()
                }
            }

            genreInput.onblur = thisEvent => {
                // on-blur, remove
                clearOldSearch(thisEvent.srcElement.getAttribute("id"))
            }

            genreInput.onfocus = thisEvent => {
                // on-focus, replace
                searchDropDown(thisEvent.srcElement.getAttribute("id"), genres, "genre")
            }

            genreInput.onkeyup = thisEvent => {
